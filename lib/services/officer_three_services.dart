...
// Sign Up
  Future<bool?> signUp(OfficerThreeModel user) async {
    try {
      ...
      await FirebaseFirestore.instance.collection('cheif').doc(userCredential.user!.uid).set(usermodel.toMap());
      return true;
    } catch (e) {
...
// Sign In
  ...
  Future<List<OfficerThreeModel>> viewAllOfficers() async {
    try {
      QuerySnapshot<Map<String, dynamic>> snapshot = await FirebaseFirestore.instance.collection('cheif').get();
